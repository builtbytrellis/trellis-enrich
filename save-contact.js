...
function mergeContact(existing, incoming) {
  ...
  // Notes are additive — append incoming (e.g. voice memos) instead of dropping when notes already exist
  ...
  // A merge resolves the contact — clear any review flag
...
module.exports = async (req, res) => {
  ...
  const { contact, targetAgentId, targetContactId } = req.body;
  ...
  try {
    ...
    // ── EXACT BIND: explicit target overrides name-matching ──
    if (targetContactId) {
      const raw = await redis.get(targetContactId);
      if (raw) {
        const existing = typeof raw === 'string' ? JSON.parse(raw) : raw;
        // confirm the record belongs to this agent
        if (existing.agentId === agentId) {
          const merged = mergeContact(existing, contact);
          merged.savedAt = existing.savedAt;
          merged.agentId = agentId;
          await redis.set(targetContactId, JSON.stringify(merged));
          return res.status(200).json({ success: true, contactId: targetContactId, action: 'merged', bind: 'exact' });
        }
      }
      // target missing or not owned — fall through to normal flow
    }
