  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const list = agents.filter(Boolean)
        ...
        .map(a => ({ agentId: a.agentId, name: a.name, email: a.email, role: a.role, createdAt: a.createdAt }));
    ...
    if (req.method === 'POST') {
      ...
      const agent = { agentId, name, email: email.toLowerCase().trim(), passwordHash, salt, role, createdAt: new Date().toISOString() };
